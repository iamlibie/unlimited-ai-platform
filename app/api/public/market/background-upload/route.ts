...
export async function POST(req: Request) {
  ...
  try {
    processedBuffer = await sharp(sourceBuffer, {
      ...
      .rotate()
      .resize(900, 1200, { fit: "cover", withoutEnlargement: true })
      .webp({ quality: 84 })
      .toBuffer();
  } catch {
    return NextResponse.json(
  ...
  const uploadDir = await resolveRoleUploadDir("role-backgrounds");
  ...
  return NextResponse.json({
    data: {
      url: buildRoleUploadUrl("role-backgrounds", fileName),
